...
const dockTypes = require('@docknetwork/sdk/types.json');
...
class BackendV3 {
  ...
  async getPolkadotAPI() {
    ...
    if (this.config.dockTypes) {
      logger.info(`Using dock network types, count: ${Object.keys(dockTypes).length}`);
    ...
    const api = await ApiPromise.create(apiParams);
    await api.isReady;
