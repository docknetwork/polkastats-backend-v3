  ...
  harvestBlocks: async function(api, pool, startBlock, endBlock) {
    while (endBlock >= startBlock) {
      ...
      try {
        ...
        const ChainCurrentSlot = 0;
        const ChainEpochIndex = 0;
        const ChainGenesisSlot = 0;
        const eraElectionStatus = 'Close';
        const ChainCurrentEra = 0;
        ...
        const [
          ...
          api.query.poAModule.activeValidators.at(blockHash),
        ...
        const validatorCount = validatorList.length;
        ...
        const epochDuration = new BigNumber(2400);
        // const sessionsPerEra = new BigNumber(api.consts.staking.sessionsPerEra);
        const sessionsPerEra = 6; // TODO: use real value?
